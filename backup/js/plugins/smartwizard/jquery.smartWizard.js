...
function SmartWizard(target, options) {
            ...
            if(!$(this).hasClass('buttonDisabled')){
                if($.isFunction($this.options.onFinish)) {
                    ...
                    if(frm && frm.length){
                        ...
                        var st = $($this.target).find("ul > li").length;
                        $($this.target).find("a[rel="+(st-1)+"]").addClass("active");
                        $($this.target).find("a[rel="+st+"]").addClass("active");
    ...
    var _setupStep = function($this, curStep, selStep) {
        ...
        if($(curStep).attr("rel") > 1){
            var stepn = parseInt($(curStep).attr("rel"));            
            $($this.target).find("a[rel="+(stepn-1)+"]").addClass("active");
            
            if($(selStep).attr("rel") == $($this.target).find("ul > li").length){                
                $(selStep).parent("li").prev("li").find("a").addClass("active");
                $(selStep).addClass("active");
            }
        }
    ...
    var _adjustButton = function($this) {
        if (! $this.options.cycleSteps){
            if (0 >= $this.curStepIdx) {
                $($this.buttons.previous).addClass("buttonDisabled disabled");
                if ($this.options.hideButtonsOnDisabled) {
                    $($this.buttons.previous).hide();
                }
            }else{
                $($this.buttons.previous).removeClass("buttonDisabled disabled");
                if ($this.options.hideButtonsOnDisabled) {
                    $($this.buttons.previous).show();
                }
            }
            if (($this.steps.length-1) <= $this.curStepIdx){
                $($this.buttons.next).addClass("buttonDisabled disabled");
                if ($this.options.hideButtonsOnDisabled) {
                    $($this.buttons.next).hide();
                }
            }else{
                $($this.buttons.next).removeClass("buttonDisabled disabled");
                if ($this.options.hideButtonsOnDisabled) {
    ...
    SmartWizard.prototype.enableFinish = function (enable) {
        ...
        if (this.options.includeFinishButton){
            if (!this.steps.hasClass('disabled') || this.options.enableFinishButton){
                $(this.buttons.finish).removeClass("buttonDisabled disabled");
                if (this.options.hideButtonsOnDisabled) {
                    $(this.buttons.finish).show();
                }
            }else{
                $(this.buttons.finish).addClass("buttonDisabled disabled");
                if (this.options.hideButtonsOnDisabled) {
